...
sealed class ItemsMenuLateral(val icon: ImageVector, val title: String, val ruta: String) { object ItemMenuNovedades: ItemsMenuLateral(icon = Icons.Outlined.Campaign, title = "Novedades", ruta = NavRoutes.NovedadesScreen)
   ...
   object ItemMenuSolicitudes: ItemsMenuLateral(icon = Icons.Outlined.AdsClick, title = "Solicitudes", ruta = NavRoutes.SolicitudesScreen)
   object ItemMenuGestion: ItemsMenuLateral(icon = Icons.Outlined.Book, title = "Gestión", ruta = NavRoutes.GestionScreen)
}